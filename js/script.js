...
// Utility to toggle button disable
const toggleButtonDisable = (button) => {
    button.disabled = !button.disabled;
}

// Function to play the joke using text-to-audio
function sayTheJoke(joke) {
    VoiceRSS.speech({
        key: '4f2aa6a5df424883825e66d2b8da7db1',
        src: joke,
        hl: 'en-us',
        v: 'Linda',
        r: 0,
...
// Function to fetch the joke from Joke API
async function fetchTheJoke() {
    let joke = 'Failed to fetch';
    try {
...
// Function triggered on click of button
async function tellAJoke() {
    let jokeString = await fetchTheJoke();
    toggleButtonDisable(button);
    sayTheJoke(jokeString);
}

button.addEventListener('click', tellAJoke);

audioElement.addEventListener('ended', toggleButtonDisable.bind(this, button));